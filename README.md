...
- Shows a live multi-line status card on its own heartbeat, using the account email total, animated waiting dots, archived coverage, new downloads as workers finish messages, recent raw-email transfer speed, elapsed time, a warmed-up remaining-time estimate, and a grouped final summary.
